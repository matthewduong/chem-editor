...
function normalizeBondAtoms(a1: number, a2: number): [number, number] {
  return a1 < a2 ? [a1, a2] : [a2, a1];
}

function getViewerBondAtoms(state: AppState, bondId: string): [number, number] | null {
  const bond = state.bonds.find((entry) => entry.id === bondId);
  if (!bond) return null;
  const fromIdx = state.viewerAtomIndexById[bond.from];
  const toIdx = state.viewerAtomIndexById[bond.to];
  if (fromIdx === undefined || toIdx === undefined) return null;
  return normalizeBondAtoms(fromIdx, toIdx);
}

export function selectCanvasHoveredViewerAtomIdx(state: AppState): number | null {
  if (!state.hoveredCanvasAtomId) return null;
  return state.viewerAtomIndexById[state.hoveredCanvasAtomId] ?? null;
}

export function selectCanvasSelectedViewerAtomIndices(state: AppState): number[] {
  const mapped = Array.from(state.selectedAtomIds)
    .map((atomId) => state.viewerAtomIndexById[atomId])
    .filter((idx): idx is number => idx !== undefined)
    .filter((idx, index, arr) => arr.indexOf(idx) === index);
  const visibleAtomCount = Object.keys(state.viewerAtomIndexById).length;
...
export function selectCanvasSelectedViewerAtomIdx(state: AppState): number | null {
  const selectedIndices = selectCanvasSelectedViewerAtomIndices(state);
  if (selectedIndices.length === 0) return null;
  if (
    state.lastInteractedAtomId &&
    state.viewerAtomIndexById[state.lastInteractedAtomId] !== undefined
  ) {
    return state.viewerAtomIndexById[state.lastInteractedAtomId];
  }
  return selectedIndices[0] ?? null;
}

export function selectCanvasHoveredViewerBondAtoms(state: AppState): [number, number] | null {
  if (!state.hoveredCanvasBondId) return null;
  return getViewerBondAtoms(state, state.hoveredCanvasBondId);
}

export function selectCanvasSelectedViewerBondAtoms(state: AppState): Array<[number, number]> {
  return Array.from(state.selectedBondIds)
    .map((bondId) => getViewerBondAtoms(state, bondId))
...
export function selectActiveViewerHighlightState(state: AppState) {
  const selectedAtomIndices = selectCanvasSelectedViewerAtomIndices(state);
  const selectedAtomIdx = selectCanvasSelectedViewerAtomIdx(state);
  return {
    hoveredAtomIdx: selectCanvasHoveredViewerAtomIdx(state) ?? state.viewerHoveredAtomIdx,
    selectedAtomIdx: selectedAtomIdx ?? state.viewerSelectedAtomIdx,
    selectedAtomIndices:
      selectedAtomIndices.length > 0
        ? selectedAtomIndices
        : state.viewerSelectedAtomIdx !== null
          ? [state.viewerSelectedAtomIdx]
          : [],
    hoveredBondAtoms: selectCanvasHoveredViewerBondAtoms(state),
    selectedBondAtoms: selectCanvasSelectedViewerBondAtoms(state),
  };
}
